"""
S42 Production Suite â€” Audio Reactive FX v4.1
================================================
...
EFFECTS:
  pulse       â€” scale/zoom pulse with audio envelope
  neon        â€” glowing edge highlight driven by audio
  cascade     â€” echo/shadow copies offset by envelope
  flicker     â€” color flash overlay on beats
  rotation    â€” rotate frame by envelope amount
  warp        â€” radial ripple warp

SMOOTHING:
  envelope_ema, attack, release, curve â€” shape the audio response
  lfo_enabled â€” optionally blend an LFO with the audio envelope

EXTERNAL INTEGRATION:
  Plug in S42P Beat Analyzer or S42P Audio Analyser outputs:
  external_envelope, external_beats â€” override internal analysis
...
class S42PAudioReactiveFX:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "images": ("IMAGE", {"tooltip": "Image batch or video frames to apply effects to. [B,H,W,C] float32."}),
                "audio":  ("AUDIO",),
            },
            "optional": {
                "mask": ("MASK", {"tooltip": "Optional foreground mask â€” effects apply only where mask > 0"}),
...
NODE_DISPLAY_NAME_MAPPINGS = {"S42PAudioReactiveFX": "S42P Audio Reactive FX ðŸŽµ"}
